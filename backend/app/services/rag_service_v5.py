...
Provide a corrected response:"""
    ...
    def _build_success_response(self, ctx: PipelineContextV5) -> RAGResponseV5:
        ...
        # MANDATORY: Ensure Risk Assessment module appears for single_candidate queries
        # This is a FAILSAFE in case orchestrator doesn't add it
        if query_type == "single_candidate" and "### ⚠️ Risk Assessment" not in formatted_answer:
            logger.info("[RAG] Adding MANDATORY Risk Assessment module (failsafe)")
            risk_section = self._build_risk_assessment_failsafe(ctx.effective_chunks, candidate_name)
    ...
    # =========================================================================
    # RISK ASSESSMENT MODULE (MANDATORY FOR SINGLE CANDIDATE)
    # =========================================================================
    
    def _build_risk_assessment_failsafe(self, chunks: List[Dict[str, Any]], candidate_name: str = None) -> str:
        """
        Build Risk Assessment section - FAILSAFE method.
        
        MANDATORY MODULE with 5 components:
        ...
        This method is called as a FAILSAFE when orchestrator doesn't add the section.
        """
        # Default values
        score = 0.0
        tenure = 0.0
        ...
        # Find chunks for target candidate
        target_chunks = chunks or []
        if candidate_name and chunks:
            target_chunks = [
                c for c in chunks 
                if candidate_name.lower() in c.get("metadata", {}).get("candidate_name", "").lower()
            ]
        ...
        for chunk in target_chunks:
            ...
            if score > 0 or exp > 0 or tenure > 0:
                break
